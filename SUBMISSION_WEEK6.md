...
- **Bouncy Castle 1.70** - Cryptographic support for AES-GCM and HMAC-related operations
...
Java 17 was selected based on the Maven build target and the requirements for:
- **Cross-platform support** (Swing works uniformly on all OS)
- **Strong cryptography libraries** (AES-GCM encryption + HMAC integrity checks)
...
- **Profile placeholder**: notebook context is currently represented through the default/editable notebook field; a full user profile is future scope.
- **Encryption Status**: Unlocked on startup with a documented demo password for the course prototype
...
- REQ-SEC-1: Encrypt body at rest using AES-GCM
...
### Slice 2: Retrieve, List, and Soft-Delete Notes (REQ-2, REQ-4, Story 2/4)

**Requirements Realized**:
- REQ-2: Read note by ID and display decrypted content
- REQ-4: Soft-delete note and remove it from normal list/read flows
...
3. `SQLiteNoteStorage.get(noteId)`:
   ...
   - Verifies HMAC before returning decrypted content
...
| REQ-3 | Update with versioning and timestamps | | Partial | Storage-level `SQLiteNoteStorage.update()` exists and is tested; edit UI is future work |
...
| REQ-SEC-1 | Encryption at rest | ✓ | ✓ | AES-GCM in `EncryptionManager` |
...
| 3 | Update with versioning | storage | ✓ Storage API increments version; edit UI not yet exposed | **PARTIAL** |
| 4 | Soft-delete note | 2 | ✓ Marked deleted, removed from list | **DONE** |
| 5 | Search efficiently | storage | ✓ FTS5 search implemented and benchmarked on current test dataset | **PARTIAL** |
...
- **Change made**: 
  ...
  - Tightened PBKDF2 key derivation beyond Copilot's initial suggestion (65,536 iterations)
...
- **What I changed (important)**:
  - **Issue Found**: Copilot's first suggestion used weak/default cipher patterns → I changed to AES-GCM with random IVs and password-based key derivation
  - Implemented PBKDF2 instead of raw SHA-256 hashing
...
✓ **Tests Pass**: `mvn test` passes 19 unit, integration, and performance tests  
...
4. **Weakness**: Schema versioning is present but migration framework is still minimal
   - **Impact**: Difficult to evolve database schema
   - **Fix Made**: Added `schema_version` table and startup migration check
   - **Future**: Add explicit migration scripts for future schema changes
...
✓ **Tests**: 19 tests covering model, storage, integration, export/import, purge, HMAC, and search performance  
✓ **Traceability**: Requirements linked to code + UML  

### Known Limitations (for Week 7)

- [x] Search (REQ-5) implemented and benchmarked on a 2k-note test dataset
- [x] Plugin hooks (REQ-7) interface and plugin state persistence implemented
- [x] Export/import (REQ-8) implemented and covered by integration test
- [ ] Schema migration (GOV-1) has version tracking but no multi-version migration scripts yet
...
- Is the encryption approach sufficient (AES-GCM note-body encryption + HMAC)?
